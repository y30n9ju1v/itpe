...
단일 DBMS 서버로는 처리할 수 없는 초대용량 데이터와 초고속 트랜잭션 요구를 해결하기 위해 데이터를 여러 서버에 분산하는 전략이 필수가 되었습니다. 샤딩(Sharding, 데이터를 여러 독립 DB 서버로 수평 분산)은 성능 향상을, 리플리케이션(Replication, 동일 데이터를 여러 서버에 복제)은 가용성과 읽기 부하 분산을 목적으로 하며, 두 기법을 조합하여 실제 대규모 시스템을 구성합니다.
