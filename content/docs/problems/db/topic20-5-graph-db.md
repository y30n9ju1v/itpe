...
| Property Graph | 쿼리 언어 | Cypher(Neo4j), Gremlin(Apache TinkerPop), **GQL**(ISO/IEC 39075, 2024년 4월 국제 표준) |
...
**Neo4j 5.x 분산 아키텍처**
- **챌린지**: 그래프 데이터는 노드 간 연결이 전체에 걸쳐 있어 RDBMS처럼 행 단위로 깔끔하게 샤딩하기 어렵고, 그래프 분할을 잘못하면 샤드 간 엣지 탐색이 빈번해져 분산 효과가 반감됨. Neo4j 4.x의 Fabric은 5.x에서 **Composite Database**로 대체되어 복수 데이터베이스를 단일 쿼리로 조회하는 방식으로 재설계됨
