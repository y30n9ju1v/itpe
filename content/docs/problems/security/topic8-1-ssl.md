...
| 위협 | 방어 방법 |
|------|-----------|
| 재전송 공격(Replay Attack) | Early-Data 헤더 확인, 서버에서 Nonce/토큰으로 중복 요청 탐지 |
| 멱등하지 않은 요청에 0-RTT 사용 | GET 요청에만 0-RTT 허용, POST·결제 요청은 1-RTT 강제 |
| 세션 티켓 탈취 | 세션 티켓 유효기간 최소화(24시간 이하), 주기적 키 로테이션 |

2) QUIC / HTTP3 보안

```
QUIC = UDP + TLS 1.3 (전송계층에 TLS 통합)
HTTP/3 ──[QUIC]──▶ 0-RTT 재연결, 헤드오브라인 블로킹 제거
                    TLS 1.3 필수, 알고리즘: ChaCha20-Poly1305 or AES-GCM
```

- QUIC는 TLS 1.3을 프로토콜 자체에 내장하여 별도 핸드셰이크 없이 암호화를 제공하며, 기존 WAF·IDS가 QUIC 패킷을 UDP로 오인해 검사를 우회할 수 있어 **QUIC 트래픽 가시성 확보**가 새로운 보안 과제임.

3) 인증서 핀닝 (Certificate Pinning)

- **목적**: 중간자 공격(MITM)에서 CA 침해 시에도 특정 공개키·인증서만 신뢰하도록 고정
- **방식**: 앱 내부에 기대하는 공개키 해시를 하드코딩 → 서버 인증서와 불일치 시 연결 차단
- **한계**: 인증서 갱신 시 앱 업데이트 필요(핀 만료 문제) → 루트 CA·중간 CA 핀닝으로 완화

- TLS 1.3으로의 전환은 PFS(Perfect Forward Secrecy) 필수화, 취약 알고리즘 제거, QUIC 통합으로 이어지며, 인증서 자동화(ACME)와 CT 로그 감사를 결합한 체계적인 PKI 운영이 현대 보안 통신의 기반임.  "끝"
