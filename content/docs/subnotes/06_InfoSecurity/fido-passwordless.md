...
| **차별화** | **차세대 패스키(Passkey) 아키텍처 및 하드웨어 TEE 보안 연동 전략**<br>1. **패스키 (Passkey - 동기화된 FIDO 자격증명) 도입**: 기존 FIDO는 기기 귀속형(Device-bound) 자격증명을 사용하여 단말기 교체/분실 시 재인증 절차가 매우 번거로움. 이를 해결하기 위해 사용자의 FIDO 비대칭 키 쌍을 엔드투엔드 암호화로 보호되는 클라우드 키체인(iCloud, Google)을 통해 다중 단말에 자동 동기화하는 **Passkey** 도입.<br>2. **하드웨어 격리 영역 (TEE / SE) 연동을 통한 물리 탈취 차단**: 단말이 루팅되거나 맬웨어에 감염되더라도 개인키와 생체 지문 정보가 덤프되지 않도록, 메인 커널과 하드웨어적으로 독립 실행되는 **TEE (신뢰 실행 환경)** 내부에서 서명 연산 처리 강제.<br>3. **패스워드리스 도입에 따른 피싱 공격(Phishing-resistant) 원천 방어**: FIDO는 도메인 이름을 비대칭 키 서명 검증의 팩터로 포함하므로, 사용자가 피싱 위조 사이트(예: naver-fake.com)에 속아 지문을 인식하더라도 서버 측 도메인 불일치로 서명이 실패하여 계정 도용이 기술적으로 차단됨.<br>4. **무자각 지속인증(Transparent Continuous Authentication)으로 로그인 이후 세션 보호**: FIDO/패스키가 최초 1회 강력한 인증을 담당한다면, 세션 하이재킹·내부자 위협·공유 디바이스 악용 등 로그인 이후 위협에는 타이핑 패턴·마우스 동작·보행 패턴·음성·위치(GPS/Wi-Fi)·UEBA 등 행동 생체인식을 사용자 개입 없이 지속 분석해 신뢰 점수(Trust Score)를 실시간 산정하는 지속인증이 보완. 초기 학습기간(2~4주) 동안 기준 프로파일을 수립한 뒤, 고위험 시 추가 인증 요구·세션 종료, 중위험 시 모니터링 강화, 정상 시 투명 허용의 3단계로 대응하며, 멀티모달 융합과 적응형 임계값으로 오인식을 보완하고 행동 데이터는 로컬(Edge AI) 처리로 프라이버시를 보호. 산출된 신뢰 점수를 Zero Trust의 PEP(정책 시행 지점)에 실시간 전달하면 FIDO의 강력한 최초 인증과 지속인증의 세션 내내 검증이 결합되어 "Never Trust, Always Verify" 원칙이 완성됨. |
