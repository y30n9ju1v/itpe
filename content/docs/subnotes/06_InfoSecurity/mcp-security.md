...
| **정의** | LLM-외부도구/데이터소스 표준 연결 프로토콜 MCP → AI 에이전트에 파일시스템·DB·API 접근권한 부여 시 새롭게 열리는 **7대 보안취약점(프·권·오·악·민·S·도)**과 대응방안 |
...
| **비교** | **일반 LLM 프롬프트 인젝션**<br>- 공격표면: 대화 입력텍스트로 한정<br>- 파급력: 잘못된 응답생성에 그침 (도구실행 권한 없음)<br><br>**MCP 환경의 프롬프트 인젝션**<br>- 공격표면: 대화입력 + 연동된 파일시스템·DB·외부API 전체로 확장<br>- 파급력: 인젝션 성공 시 파일삭제·이메일발송·내부망 SSRF 등 시스템수준 피해 직결 |
| **차별화** | **MCP 도입 기업의 3단계 보안 내재화 전략**<br>1. 최소권한 + 도구 Allowlist: MCP Server가 필요 최소 파일경로·API스코프만 접근 제한 + 사용가능 도구 화이트리스트화 → 도구혼란 공격 방지<br>2. 샌드박스 격리 + 감사로그: MCP Server 컨테이너 격리실행 + 모든 도구호출 내역 기록·모니터링 → SSRF·권한상승 시도 사후추적<br>3. 고위험 작업 Human-in-the-Loop: 파일삭제·송금·외부전송 등 파괴적 작업은 사용자 명시적 승인 거치도록 설계, 신뢰된 MCP Server만 등록허용하는 검증체계 병행 |
