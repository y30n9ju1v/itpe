...
| **차별화** | **무형 SW개발 환경의 진척율 측정 한계 극복 및 Agile EVM 설계 방안**<br>1. **정량적 진척율 측정 규칙 수립**: 주관적 진척보고 방지 → 시작 50%/완료검수 50% 인정하는 **50/50 법칙** 또는 **마일스톤 산정법** 강제<br>2. **스토리포인트 기반 Agile EVM 구축**: 폭포수 WBS 대신 스프린트 Planned Points=PV, DoD 통과 Velocity=EV로 환산 → 스프린트마다 정량통제<br>3. **TCPI(잔여성과지수) 연계 현실적 목표관리**: $TCPI=\frac{BAC-EV}{BAC-AC}$(또는 $EAC-AC$) 1.10 초과 시 → 추가예산 확보(EAC 승인) 또는 범위축소 변경통제(CCB) 긴급가동 |
