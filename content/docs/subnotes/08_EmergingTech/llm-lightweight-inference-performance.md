...
| **정의** | 대형 LLM을 온디바이스 실행 규모로 경량화하는 **sLLM/양자화 기술** + LLM 추론 응답성 측정 **TTFT(첫토큰 지연)·TPOT(토큰당 생성시간) 지표** |
...
| **차별화** | **실시간 대화형 서비스 지연 체감 최소화 종합 전략**<br>1. **TTFT 최적화**: 반복 시스템프롬프트 KV캐시 재사용 Prompt Caching, Draft모델 후보토큰 예측·검증 Speculative Decoding, Prefill 전용서버 분리운영<br>2. **TPOT 최적화**: TurboQuant 등 고속양자화로 모델크기 축소 + Continuous Batching + Flash Attention → GPU 활용률 극대화<br>3. **온디바이스 배포 기준**: TTFT 200ms 이하, TPOT 50ms/token 이하 목표 → sLLM(Gemini Nano, Phi-3 Mini) + GGUF/GGML CPU추론 포맷 결합, 서버 의존無 로컬실행 |
