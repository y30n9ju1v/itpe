...
| **정의** | 레이블없는 대규모 텍스트로 자기지도학습(Self-Supervised) → 언어 통계패턴·문맥지식 사전습득 범용언어모델 **PLM**(BERT/GPT/T5)<br>→ 지시이해력+인간선호정렬 Pre-training→SFT→RLHF 3단계 추가학습 → 범용 대화·추론 **LLM** 완성 |
...
| **차별화** | **PLM 기반 LLM 구축 실무 전략**<br>1. **오픈소스 Base 활용 전략**: 자체 Pre-training=TB급 데이터·막대한 GPU비용(전체비용 90%↑) → 오픈소스 Base(LLaMA/Qwen)에 도메인특화 SFT/DPO만 집중, 비용효율화<br>2. **데이터 품질 관리**: 웹크롤링 Pre-training 데이터 편향·오류=환각(Hallucination) 근본원인 → 필터링·중복제거·품질스코어링 + RAG 결합 사실성 보완<br>3. **보상 해킹 방지**: RLHF에서 보상모델 속이는 장황응답 최적화(보상해킹) 방지 → 다수평가자 기반 보상모델+KL페널티 적용, PPO보다 단순한 DPO도 저비용 대안 확대 |
