...
| **차별화** | **초거대 모델 학습을 위한 3D 병렬화 및 인터커넥트 설계 전략**<br>1. **3D 병렬화 조합 최적화**: 데이터+모델+파이프라인 병렬화를 모델 규모별로 조합해 GPU 유휴시간(Bubble)을 최소화하고 선형에 가까운 확장성 확보.<br>2. **인터커넥트 계층 설계**: 동일 서버 내부는 NVLink(900GB/s), 서버 간은 InfiniBand/RoCE로 구성해 All-reduce 통신 병목을 제거하고 GPU 이기종 혼합을 지양.<br>3. **메모리 최적화 기법 결합**: Gradient Checkpointing으로 중간 활성화를 재계산해 메모리를 절감하고, Gradient Compression 및 통신-연산 오버랩으로 통신 병목을 추가 완화. |
