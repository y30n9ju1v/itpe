...
| **정의** | Microsoft DCOM 기반 기존 OPC의 Windows의존성·보안취약점 극복<br>→ TCP/IP 기반 플랫폼·벤더 중립 차세대 산업자동화 통신 프로토콜(IEC 62541) |
...
| **차별화** | **스마트팩토리 실시간 통합 및 디지털 트윈 연계 전략**<br>1. **수직·수평 통합 아키텍처 표준화**: ISA-95 참조모델 따라 PLC-SCADA-MES-ERP 전계층 OPC UA 통합정보모델 연결 → 이기종 벤더장비 간 상호운용성 확보<br>2. **TSN 결합 실시간 제어 고도화**: 이더넷 지터·지연 문제 → OPC UA over TSN 적용, 로봇제어·컨베이어동기화 등 밀리초 정밀제어 공정 대응<br>3. **디지털트윈 실시간 데이터 파이프라인**: 현장 PLC/센서 OPC UA 노드데이터 → Pub/Sub(MQTT) 클라우드 디지털트윈 반영 → 원격모니터링·시뮬레이션 예지보전 |
