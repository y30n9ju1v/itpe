...
| **정의** | 물리세계 인지·이해→행동 계획·수행, 현실공간 자율작동 체화(Embodied) AI **피지컬 AI**<br>+ 시각·언어입력→로봇 물리행동 직접변환 통합 파운데이션모델 **VLA 모델**(피지컬 AI의 두뇌) |
...
| **차별화** | **Sim-to-Real 격차 해소 및 VLA 계층적 제어 실무 전략**<br>1. **도메인 랜덤화+실환경 파인튜닝**: 조명·질감·물리계수 무작위화(도메인 랜덤화) → 일반화 성능↑, 실환경 소량데이터 파인튜닝으로 Sim-to-Real Gap 축소, 디지털트윈 검증 파이프라인 병행<br>2. **VLA 계층적 제어(고수준/저수준 분리)**: RT-2/GR00T 등 고수준 VLA=목표·경로 저빈도 생성, 저수준 경량컨트롤러=고빈도 실시간 관절제어 → VLA 추론지연 실시간성 제약 해소<br>3. **안전계층+단계적 실환경 배포**: 로봇 안전표준·ISO/IEC 42001 연계 Fail-safe 설계 + Staged Rollout → VLA 오작동 시 물리적 피해 최소화, 오픈 로보틱스 데이터·플랫폼 공유로 원천기술 종속 리스크 완화 |
