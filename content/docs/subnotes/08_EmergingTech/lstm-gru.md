...
| **정의** | RNN 기울기소실(Vanishing Gradient) 문제를 셀상태(Cell State)+게이트 구조로 해결, 장기의존성 보존하는 순환신경망 **LSTM** + 게이트 2개로 단순화해 연산효율 높인 **GRU** |
...
| **차별화** | **RNN 계열 모델 선택 실무 전략**<br>1. **표현력 vs 효율 트레이드오프**: 셀·은닉 상태 분리한 LSTM은 표현력↑, 연산·메모리 부담↑ → 저지연 온디바이스·임베디드는 GRU 우선검토<br>2. **Transformer 전환 판단기준**: 초장문·대규모 병렬처리 필요 → Attention/Transformer 우선검토 / 순차적·저지연 스트리밍 중요 → LSTM/GRU 선택적용<br>3. **기울기 보존 원리 응용**: 셀상태 덧셈(⊕) 경로가 기울기소실 완화 핵심원리 → 유사한 잔차연결(Residual Connection) 개념 타 심층아키텍처에도 응용 |
