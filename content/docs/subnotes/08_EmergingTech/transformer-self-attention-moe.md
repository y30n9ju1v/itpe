...
| **구성요소** | 1. **Q/K/V 행렬**: 입력 벡터에 학습 가능 가중치를 곱해 생성, 검색 쿼리·인덱스·결과에 비유<br>2. **Multi-Head Attention**: h개 헤드를 병렬 실행해 구문적·의미적 등 다양한 관계 패턴 동시 학습(GPT-3 96헤드)<br>3. **위치 인코딩(Positional Encoding)**: RNN 없이 순서 정보를 임베딩에 주입<br>4. **MoE 게이팅 네트워크**: 입력마다 전체 Expert 중 상위 K개만 선택해 연산량 절감(Sparse Activation)<br>5. **대표 모델**: Mixtral 8x7B(8개 중 2개 선택), Switch Transformer(Google)<br>6. **차등 트랜스포머(Differential Transformer)**: 하나의 헤드를 Q1K1/Q2K2 두 개의 독립 Softmax Attention Map(A1, A2)으로 계산 후, 학습 가능한 스칼라 λ로 `DiffAttn=(A1-λ·A2)·V` 차감 연산 — 노이즈 캔슬링 헤드폰처럼 두 맵의 공통 노이즈(무관 토큰에 대한 미세 가중치)를 상쇄하고 유의미한 신호만 증폭, GroupNorm으로 헤드별 스케일 보정, Needle-in-Haystack 긴 컨텍스트 검색 정확도 및 환각 감소에 효과 |
...
| **차별화** | **Attention 연산 병목(O(n²)) 및 MoE 라우팅 불균형 극복 전략**<br>1. **긴 컨텍스트 처리를 위한 효율화 Attention**: 시퀀스 길이 증가 시 폭증하는 O(n²) 연산·메모리를 완화하기 위해 Sparse Attention, Linear Attention, Flash Attention(메모리 I/O 최적화) 적용.<br>2. **MoE FFN 레이어 치환 설계**: 현대 LLM은 Transformer 블록의 FFN을 MoE로 교체해 파라미터는 늘리되(확장성) 실제 활성 연산량은 유지하는 구조로 추론 비용 절감.<br>3. **√dk 스케일링을 통한 기울기 소실 방지**: Key 차원수(dk)가 클수록 내적 값이 커져 Softmax 기울기가 소실되므로 QKᵀ를 √dk로 나누는 스케일링을 필수 적용.<br>4. **차등 트랜스포머 도입 시 실무 고려사항**: λ 초기화·레이어별 민감도는 논문 스케줄(λ_init=0.8-0.6e^(-0.3·layer)) 기반 A/B 검증, 기존 BERT/GPT 가중치와 호환 불가하므로 법률·장문 요약 등 긴 컨텍스트·사실성(Factuality) 요구가 큰 특화 도메인에 선별 적용하며, "노이즈 제거=환각 완전 해결"로 과대 해석하지 않도록 RAG·Fact-Checking 파이프라인과 병행. |
