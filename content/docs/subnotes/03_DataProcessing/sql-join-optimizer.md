...
| **정의** | 릴레이션 입력→수학적 연산으로 새 릴레이션 생성하는 절차적 질의언어 관계대수 + 이를 구현한 SQL 논리적 JOIN·물리 조인 알고리즘(NL/SM/Hash) + 선언형 SQL을 최소비용 실행계획으로 변환하는 옵티마이저(RBO/CBO) 통합 체계 |
...
| **차별화** | **JOIN·옵티마이저 성능 최적화 실무 전략**<br>1. **데이터 특성 기반 알고리즘 선택**: 소량+인덱스 완비 OLTP→NL Join(USE_NL 힌트) / 대용량 등가 집계→Hash Join(USE_HASH) / 인덱스 부재·비등가→Sort Merge Join(USE_MERGE) 힌트 강제 제어<br>2. **카디널리티 추정 정확도 관리**: CBO 성능=통계정보 정확도 의존 → 대량 변경 후 히스토그램·NDV 주기적 갱신 → 조인 순서 오판 방지<br>3. **Outer Join 오용 방지·Driving 테이블 선정**: LEFT/RIGHT OUTER JOIN=조인 순서 고정 → 최적화 폭 제한, 필요시에만 사용 - NL Join 시 결과 작은 테이블을 Driving으로 선정<br>4. **분산 쿼리 엔진 연계**: Spark SQL·Trino 등 분산 엔진도 동일 관계대수 변환 원리(Predicate Pushdown, 조인 재정렬)로 실행계획 수립 → 대용량 분산환경도 동일 최적화 원칙 적용 |
