...
| **차별화** | **RAG 시스템 구축 시 검색 정확도(Recall)와 효율성 극대화 방안**<br>1. **의미적 청킹(Semantic Chunking)**: 글자수 단위 대신 문맥오버랩(50~100 토큰) 둔 256~512 토큰 청킹 + Parent-Child Retrieval 구조 → 정보품질 확보<br>2. **하이브리드 RRF(Reciprocal Rank Fusion) 재순위화**: Dense+Sparse 검색 혼합 후 Re-ranker(예: Cohere Re-rank) 적용 → 할루시네이션 최소화<br>3. **인덱스 파라미터 최적화**: HNSW `ef_construction`·`M` 값 튜닝 → Recall@K vs QPS(Queries Per Second) 트레이드오프 조정 |
