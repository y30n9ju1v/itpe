...
| **구성요소** | 1. **데이터 품질 차원 (유·일·정·유·일·접)**: 유효성, 일관성, 정확성, 유일성, 일치성, 접근성 등의 지표<br>2. **데이터 프로파일링**: 컬럼의 데이터 분포, 결측치, 형식 오류 등을 SQL로 분석하는 프로파일링 기법<br>3. **ISO 8000 표준 (Part 110)**: 기계 가독성(구문), 데이터 딕셔너리 정합성(의미), 스키마 요건 충족(적합성)<br>4. **데이터 옵저버빌리티**: 데이터 흐름(Lineage), 스키마 변경, 지연(Freshness), 볼륨 변화 등을 실시간 관측 |
