...
| **정의** | 대용량·다양·고속의 빅데이터를 수집·저장·처리·분석·활용하기 위해 서로 다른 처리 엔진을 조합하는 폴리글랏 프로세싱 패러다임과, 이를 구현하는 대표 아키텍처인 람다(배치+스트림 병행)·카파(스트림 단일) 아키텍처, 그리고 분산 저장·처리의 핵심인 하둡 에코시스템(HDFS)과 실시간 메시징 시스템 카프카(Kafka)를 포괄하는 빅데이터 플랫폼 기술 체계 |
...
| **차별화** | **빅데이터 플랫폼 설계 및 운영 전략** <br>1. **아키텍처 선택 기준**: 정확성·일관성이 최우선(금융 정산 등)이면 람다, 운영 단순성·개발 속도가 중요하면 카파를 채택하고 Kafka를 단일 소스 오브 트루스로 활용<br>2. **HDFS 복제 전략**: 기본 3중 복제(Replication Factor 3)로 DataNode 장애 시 자동 복구, NameNode는 SPOF이므로 HA(Active-Standby) 구성 필수<br>3. **Kafka 확장성**: 브로커 3대에서 시작해 온라인 상태에서 서비스 중단 없이 수십 대로 Scale-out 가능하며, 파티션 단위 순서 보장·리플리케이션 팩터 조정으로 신뢰성 확보<br>4. **클라우드 마이그레이션**: AWS EMR·Azure HDInsight·GCP Dataproc 등 관리형 하둡/스파크 서비스로 전환하여 운영 부담 경감 |
