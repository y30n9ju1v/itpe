...
| **정의** | 클라이언트-서버 간 하이퍼텍스트 전송용 무상태(Stateless) 요청-응답 프로토콜, TCP기반 HTTP/1.1·2의 HOL Blocking 한계 → UDP기반 QUIC 채택한 HTTP/3(RFC 9114)로 진화 |
...
| **구성요소** | 1. **HTTP 버전별 혁신**: 1.0(1996, 요청마다 새연결) → 1.1(1997, Keep-Alive) → 2(2015, 멀티플렉싱·HPACK·서버푸시) → 3(2022, QUIC 기반)<br>2. **QUIC 4대 혁신**: 연결설정(1RTT/0-RTT, 기존TCP+TLS 2~3RTT) / HOL차단 해소(스트림별 독립 손실복구) / 연결이동(Connection ID로 IP변경 후 세션유지) / 암호화(TLS 1.3 프로토콜레벨 내장)<br>3. **QUIC 스택**: [HTTP/3]-[QUIC(TLS1.3 내장)]-[UDP]-[IP] vs [HTTP/2]-[TLS1.3]-[TCP]-[IP]<br>4. **헤더압축**: HTTP/2는 HPACK, HTTP/3는 QPACK |
...
| **차별화** | **HTTP/3 도입 및 모바일 최적화 실무 전략**<br>1. **UDP 443 방화벽 사전검토**: 기업방화벽 UDP 443 차단 시 HTTP/2 자동폴백 → 방화벽 허용정책·CDN QUIC 지원 사전확인, Alt-Svc 헤더 기반 점진 마이그레이션<br>2. **모바일 Connection Migration**: Wi-Fi↔LTE 핸드오버 시 Connection ID 기반 세션연속성 보장, TCP 재수립 지연 제거<br>3. **레거시 HTTP/2 단계적 전환**: nginx http2 지시자 무중단 전환, HPACK 압축(반복 요청헤더 60~80% 절감)+서버푸시로 초기로드 단축 |
