...
| **정의** | 디지털 통신에서 여러 채널을 계층적으로 다중화하여 고속 전송을 실현하는 표준 체계로, 준동기식 **PDH**에서 완전 동기식 **SDH**(유럽/국제), **SONET**(북미)으로 발전한 광통신 네트워크의 기반 기술 |
...
| **차별화** | **레거시 PDH 망에서 SDH/SONET 백본으로의 전환 및 차세대 연계**<br>1. **포인터 바이트 기반 정렬(Pointer Justification)**: PDH의 독립 클럭으로 인한 주파수 편차 문제를, SDH는 단일 기준 클럭 동기화와 포인터 바이트로 해결해 정렬(stuffing) 절차를 없애고 중간 노드에서 특정 채널을 직접 추출 가능하게 함.<br>2. **링 토폴로지 기반 고가용성 설계**: BLSR/UPSR 링 구조를 채택해 광케이블 단선 등 장애 발생 시 50ms 이내 자동 우회 복구를 실현, 통신사 백본망의 고가용성 요건을 충족.<br>3. **차세대 전송망과의 연계**: SDH/SONET 회선 교환 기반 인프라는 이후 패킷 기반 광전송(OTN, WDM)과 5G/6G 프론트홀·백홀 전송으로 발전하는 기반이 됨. |
