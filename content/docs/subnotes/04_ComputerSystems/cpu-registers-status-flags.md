...
| **정의** | Fetch-Decode-Execute 과정에 쓰는 초고속 내부 저장소 **레지스터** + ALU 결과·CPU상태 표시 → 조건분기 판단근거 **상태레지스터(SR/PSW) 조건플래그** |
...
| **차별화** | **CMP 플래그 갱신 원리 및 컨텍스트 스위칭 연계**<br>1. **CMP 연산 원리**: `CMP A,B` → 내부 A-B 수행, 결과값 버림, N/Z/C/V만 갱신 → BEQ(Z=1)·BGT(Z=0∧N=V)·BLT(N≠V) 판별근거<br>2. **컨텍스트 저장**: 시스템콜/인터럽트 시 PC·SR 등 전체 레지스터 → 스택 저장(Context Save) → 복귀 시 실행흐름·플래그 복원<br>3. **스택프레임 SP/FP 연계**: 함수호출마다 SP이동 → 매개변수·반환주소·지역변수 담는 프레임 생성, FP는 고정기준주소로 지역변수 접근 단순화 |
